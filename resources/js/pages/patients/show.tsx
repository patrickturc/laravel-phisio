...
export default function PatientShow({
    ...
    const evolutionsList = [...patient.evolutions].sort(
        (a, b) =>
            new Date(b.data_atendimento).getTime() -
            new Date(a.data_atendimento).getTime(),
    );
    const appointmentsList = [...patient.appointments].sort(
        (a, b) =>
            new Date(b.appointment_date).getTime() -
            new Date(a.appointment_date).getTime(),
    );
    const financialList = [...(patient.financial_transactions || [])].sort(
        (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime(),
    );

    const missedAppointments = appointmentsList.filter(
        (app) => (app.pivot?.status || app.status) === 'missed',
    );
    ...
    const isTxOverdue = (t: { status: string; due_date: string | null }) =>
        t.status === 'pending' &&
        !!t.due_date &&
    ...
    const tabs: {
        ...
        },
        {
            ...
            count: patient.memberships?.length,
        },
        {
            key: 'financial',
            label: 'Financeiro',
            icon: <DollarSign className="size-4" />,
            count: financialList.length,
        },
        {
            key: 'evolutions',
            label: 'Prontuário',
            icon: <Activity className="size-4" />,
            count: evolutionsList.length,
        },
        {
            key: 'appointments',
            label: 'Agendamentos',
            ...
            <div className="mx-auto flex h-full w-full max-w-5xl flex-1 flex-col gap-6 p-6 md:p-10">
                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="flex size-16 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/20 to-emerald-500/20 text-2xl font-bold text-primary shadow-inner">
                            ...
                            <div>
                                ...
                                <div className="mt-1 flex flex-wrap items-center gap-2">
                                    ...
                                    <span
                                        className={`inline-flex rounded-full px-3 py-0.5 text-xs font-semibold ${patient.type === 'pilates' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400'}`}
                                    >
                                        {can('patients.manage.edit') ? (
                                            <InlineEdit
                                                value={patient.type}
                ...
                <div className="flex flex-wrap gap-3">
                    ...
                    {can('evolutions.manage.create') && (
                        <button
                            ...
                            className="flex items-center gap-2 rounded-xl bg-indigo-500/10 px-4 py-2 text-sm font-semibold text-indigo-600 transition-colors hover:bg-indigo-500/20"
                        >
                    ...
                    {can('memberships.manage.create') && (
                        <button
                            onClick={() => setIsMembershipSheetOpen(true)}
                            className="flex items-center gap-2 rounded-xl bg-emerald-500/10 px-4 py-2 text-sm font-semibold text-emerald-600 transition-colors hover:bg-emerald-500/20"
                        >
                ...
                {/* Tabs */}
                <div className="flex flex-wrap gap-1 rounded-xl border border-border/30 bg-muted/30 p-1">
                    {tabs.map((tab) => (
                        <button
                            key={tab.key}
                            onClick={() => setActiveTab(tab.key)}
                            className={`flex min-w-[140px] flex-grow items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-medium transition-all sm:flex-1 ${activeTab === tab.key ? 'bg-card text-foreground shadow-sm' : 'text-muted-foreground hover:bg-card/50 hover:text-foreground'}`}
                        >
                            {tab.icon}
                            <span>{tab.label}</span>
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                            ...
                            {missedAppointments.length > 0 && (
                                <button
                                    ...
                                    className="flex cursor-pointer items-center gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-5 text-left shadow-sm transition-all hover:border-amber-500/30 hover:bg-amber-500/10 hover:shadow active:scale-95 dark:bg-amber-500/10 dark:hover:bg-amber-500/20"
                                >
                        ...
                        <div className="rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl">
                            <h3 className="mb-4 flex items-center gap-2 font-bold text-foreground">
                                <User className="size-4 text-primary" /> Dados
                                Pessoais
                            </h3>
                            <div className="grid grid-cols-1 gap-x-8 gap-y-4 text-sm md:grid-cols-2 lg:grid-cols-3">
                                {patient.gender && (
                                    <div>
                        ...
                        {(patient.emergency_contact_name ||
                            ...
                            <div className="rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl">
                                ...
                                {patient.health_notes && (
                                    <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm dark:border-amber-800/30 dark:bg-amber-900/10">
                                        <p className="mb-1 text-xs font-semibold text-amber-700 dark:text-amber-400">
                                            ⚠️ Observações Clínicas
                                        </p>
                                        <p className="whitespace-pre-line text-amber-900 dark:text-amber-300">
                                            {patient.health_notes}
                                        </p>
                                    </div>
                                )}
                    ...
                    >
                        <div className="flex items-center justify-between">
                            ...
                            {can('memberships.manage.create') && (
                                <button
                                    ...
                                    className="flex h-9 items-center gap-2 rounded-xl bg-primary px-4 text-sm font-medium text-primary-foreground shadow-sm transition-colors hover:bg-primary/90"
                                >
                                    <Plus className="size-4" /> Nova Matrícula
                                </button>
                            )}
                        ...
                        patient.memberships.length > 0 ? (
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                {patient.memberships.map((membership) => (
                                    <Link
                                        ...
                                        className="group relative overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br from-card to-card/50 p-5 shadow-sm transition-shadow hover:shadow-md"
                                    >
                                        <div
                                            className={`absolute top-0 left-0 h-full w-1.5 ${membership.status === 'active' ? 'bg-emerald-500' : membership.status === 'expired' ? 'bg-amber-500' : 'bg-red-500'}`}
                                        ></div>
                                            ...
                                            <div className="mt-3 border-t border-border/40 pt-3">
                                                <div className="mb-1.5 flex items-center justify-between">
                                                    ...
                                                    <span className="text-xs font-semibold text-foreground">
                                                        {
                                                            membership.sessions_used_this_month
                                                        }{' '}
                                                        de{' '}
                                                        {
                                                            membership.monthly_allowance
                                                ...
                                                <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${Math.min(100, ((membership.sessions_used_this_month ?? 0) / membership.monthly_allowance) * 100)}%`,
                                                        }}
                                                    />
                                                </div>
                                            </div>
                            ...
                            <div className="rounded-2xl border border-border/50 bg-card/60 p-12 text-center shadow-sm backdrop-blur-xl">
                                <Tag className="mx-auto mb-3 size-12 text-muted-foreground/30" />
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                            <div className="rounded-2xl border border-border/50 bg-card/60 p-5 backdrop-blur-xl">
                                ...
                                <p className="text-2xl font-bold text-emerald-600">
                                    {formatCurrency(
                                        financialSummary?.total_received ?? 0,
                                    )}
                                </p>
                            </div>
                            <div className="rounded-2xl border border-border/50 bg-card/60 p-5 backdrop-blur-xl">
                            ...
                            <div
                                className={`rounded-2xl border p-5 ${(financialSummary?.overdue_amount ?? 0) > 0 ? 'border-red-200 bg-red-50 dark:border-red-800/30 dark:bg-red-950/20' : 'border-border/50 bg-card/60 backdrop-blur-xl'}`}
                            >
                                ...
                                <p
                                    className={`text-2xl font-bold ${(financialSummary?.overdue_amount ?? 0) > 0 ? 'text-red-600' : 'text-muted-foreground'}`}
                                >
                                    {formatCurrency(
                        ...
                        <div className="rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl">
                            <div className="mb-4 flex items-center justify-between">
                                ...
                                <Link
                                    href={`/financial?search=${encodeURIComponent(patient.name)}`}
                            ...
                            {financialList.length === 0 ? (
                                <div className="rounded-xl border border-dashed border-border bg-muted/30 py-12 text-center">
                                    ...
                                    <p className="text-sm font-medium text-muted-foreground">
                                        Nenhum lançamento financeiro para este
                                        paciente.
                                    </p>
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        As mensalidades e cobranças aparecerão
                                        aqui.
                                    </p>
                                </div>
                            ) : (
                                    ...
                                    {financialList.map((t) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div
                                                    className={`text-sm font-bold whitespace-nowrap ${t.type === 'income' ? 'text-emerald-600' : 'text-red-600'}`}
                                                >
                                                    {t.type === 'income'
                                                        ? '+'
                                                        : '-'}{' '}
                                                ...
                                                <div className="flex items-center gap-1">
                                                    {t.status === 'paid' &&
                                                        t.type === 'income' && (
                                                            <button
                                                                ...
                                                                className="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-blue-500/10 hover:text-blue-600"
                                                                title="Gerar/reimprimir recibo"
                                                            >
                                                                <FileText className="size-4" />
                                                            </button>
                                                        )}
                                                    {t.status === 'pending' ? (
                                                        <button
                                                            ...
                                                            className="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-emerald-500/10 hover:text-emerald-600"
                                                        ...
                                                        <button
                                                            ...
                                                            className="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-amber-500/10 hover:text-amber-600"
                        ...
                        <div className="rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl">
                            <div className="mb-6 flex items-center justify-between">
                                <h2 className="flex items-center gap-2 text-lg font-bold">
                                    ...
                                    Prontuário
                                    <span className="ml-2 text-sm font-normal text-muted-foreground">
                                        ({evolutionsList.length} registros)
                                    </span>
                                </h2>
                                {can('evolutions.manage.create') && (
                                    <button
                                            ...
                                            >
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    <div className="mb-1 flex flex-wrap items-center gap-2">
                                                        ...
                                                        <span className="rounded-md bg-indigo-100 px-2 py-0.5 text-xs font-semibold text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400">
                                                            {tipoLabel[
                                                                evo
                                                                    .tipo_atendimento
                                                            ] ||
                        ...
                        <div className="rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl">
                            <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="flex gap-1 rounded-xl border border-border/30 bg-muted/50 p-1 text-xs">
                                        <button
                                            type="button"
                                            onClick={() =>
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 font-medium transition-all ${
                                                appointmentFilter === 'missed'
                                                    ? 'bg-amber-100 font-semibold text-amber-700 shadow-sm dark:bg-amber-900/30 dark:text-amber-400'
                                                    : 'text-muted-foreground hover:text-foreground'
                                            }`}
                                        >
                                            Faltas
                            ...
                            {appointmentsList.length === 0 ? (
                                ...
                                <div className="rounded-xl border border-dashed border-border bg-muted/30 py-12 text-center">
                                    <Calendar className="mx-auto mb-3 size-10 text-amber-500 text-muted-foreground/30" />
                                    <p className="text-sm font-medium text-muted-foreground">
                                        Nenhuma falta registrada para este
                                        paciente.
                                    </p>
                                </div>
                            ) : (
                                    ...
                                    <div className="space-y-1">
                                        {displayedAppointments.map((app, i) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    <div
                                                        className={`absolute top-5 left-2.5 size-3 rounded-full border-2 border-background ${statusDotClass}`}
                                                    />
                                                    <div className="min-w-0 flex-1">
                                                        <div className="mb-1 flex flex-wrap items-center gap-2">
                                                            <span className="text-xs font-medium text-muted-foreground">
                                                            ...
                                                            <span
                                                                className={`rounded-md px-2 py-0.5 text-xs font-semibold ${statusColor[appStatus] || 'bg-emerald-100 text-emerald-700'}`}
                                                            >
                                                                {statusLabel[
                                                                    appStatus
                                                                ] || appStatus}
                ...
                {activeTab === 'documents' && (
                    ...
                    >
                        ...
                        <div className="rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl lg:col-span-2">
                            ...
                            patient.documents.length === 0 ? (
                                <div className="rounded-xl border border-dashed border-border bg-muted/30 py-16 text-center">
                                    ...
                                    <p className="mt-1 text-xs text-muted-foreground">
                                        Contratos, termos e outros arquivos
                                        salvos aparecerão aqui.
                                    </p>
                                </div>
                            ) : (
                                <div className="divide-y divide-border/50">
                                            ...
                                            <div className="flex flex-shrink-0 items-center gap-2">
                                                ...
                                                {can(
                                                    ...
                                                    <button
                                                        ...
                                                        className="rounded-xl p-2.5 text-muted-foreground transition-colors hover:bg-red-500/10 hover:text-red-600"
                        ...
                        {can('patients.manage.edit') && (
                            <div className="h-fit rounded-2xl border border-border/50 bg-card/60 p-6 shadow-sm backdrop-blur-xl">
                ...
                {previewingDoc && (
                    ...
                    >
                        ...
                        <motion.div
                            ...
                            className="relative z-10 flex h-[90vh] w-[95vw] max-w-4xl flex-col overflow-hidden rounded-2xl border border-border/50 bg-card shadow-2xl"
                        ...
                        >
                            ...
                            <div className="flex-1 overflow-hidden bg-muted/20">
                                {previewingDoc.original_name.match(
