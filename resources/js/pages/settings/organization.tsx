...
export default function Organization({ tenant, missingFields }: Props) {
    ...
    const { data, setData, patch, processing, errors, recentlySuccessful } = useForm<TenantForm>({
        name: tenant.name ?? '',
    ...
    const fieldClass = (field: string) =>
        isMissing(field) ? 'border-amber-400 focus-visible:border-amber-500' : undefined;
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <SettingsLayout>
                <form onSubmit={submit} className="space-y-10">
                    <div className="space-y-6">
                        ...
                        {missingFields.length > 0 && (
                            <p className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-700/60 dark:bg-amber-950/40 dark:text-amber-100">
                                Os campos destacados ainda precisam ser preenchidos para concluir o cadastro.
                            </p>
                        )}
                    ...
                    <div className="space-y-6">
                        <Heading variant="small" title="Contato" description="Como falamos com a sua clínica." />
                    ...
                    <div className="space-y-6">
                        <Heading variant="small" title="Endereço" description="Informe o CEP para preencher automaticamente." />

                        <div className="grid gap-4 sm:grid-cols-[1fr_2fr]">
                            <div className="grid gap-2">
                                <Label htmlFor="cep" className="flex items-center gap-2">
                                    CEP
                                    {loadingCep && <Loader2 className="size-3.5 animate-spin text-muted-foreground" />}
                                </Label>
                                <Input
                                    id="cep"
                                    value={data.cep}
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <div className="grid gap-2">
                                <Label htmlFor="technical_manager_name">Nome</Label>
                                <Input
                                    id="technical_manager_name"
                                    value={data.technical_manager_name}
                                    onChange={(e) => setData('technical_manager_name', e.target.value)}
                                    className={fieldClass('technical_manager_name')}
                                    required
                                />
                                <InputError message={errors.technical_manager_name} />
                            </div>
                            <div className="grid gap-2">
                                <Label htmlFor="technical_manager_document">Registro (CREFITO)</Label>
                                <Input
                                    ...
                                    className={fieldClass('technical_manager_document')}
                                    required
                                />
                                <InputError message={errors.technical_manager_document} />
                            </div>
                        </div>
                    </div>
